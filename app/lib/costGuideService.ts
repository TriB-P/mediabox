...
import {
  ...
  */
  export async function addCostGuideEntry(guideId: string, entryData: CostGuideEntryFormData): Promise<string> {
    try {
        const now = new Date().toISOString();
        const newEntry = {
            ...entryData,
  ...
  * @returns {Promise<void>} Une promesse qui résout une fois la mise à jour terminée.
  */
  export async function updateCostGuideEntry(guideId: string, entryId: string, entryData: CostGuideEntryFormData): Promise<void> {
    try {
